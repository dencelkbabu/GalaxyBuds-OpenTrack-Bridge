...
public class BluetoothHeadTrackingManager : IDisposable
{
    ...
    public async Task<bool> ConnectAsync()
    {
        ...
        for (int i = 0; i < MaxRetries; i++)
        {
            try
            {
                if (i > 0) Console.WriteLine($"[INFO] Retrying connection (Attempt {i + 1}/{MaxRetries})...");
            ...
            {
                Console.WriteLine($"[ERROR] Connection attempt failed: {ex.Message}");
                if (i == MaxRetries - 1) // Last attempt
                {
                     Error?.Invoke(this, $"Connection failed after {MaxRetries} attempts: {ex.Message}");
                     return false;
                }
            }
            catch (Exception ex)
